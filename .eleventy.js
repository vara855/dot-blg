...
const { renderUnfurl } = require('./render-unfurl.js');
const lightningcssPlugin = require('@11tyrocks/eleventy-plugin-lightningcss');

function imageShortcode(src, alt = 'alt', sizes = '(min-width: 1024px) 100vw, 50vw') {
...
module.exports = function (eleventyConfig) {
    ...
    eleventyConfig.addFilter('date', value => DateTime.fromJSDate(new Date(value)).toFormat('yyyy-MM-dd'));

    eleventyConfig.addFilter('sortByDate', value => {
        return value.sort((a, b) => new Date(b.date) - new Date(a.date));
    });
    eleventyConfig.addFilter('filterTags', value => value.filter(it => it !== 'posts'));

    eleventyConfig.addFilter('onlyWithDate', value => value.filter(it => it.date));
